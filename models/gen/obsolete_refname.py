    ...
    * Note: refnamea ja reftypeä ei talleteta tekstinä, vaan kannassa tehdään
    ...
    def save(self, tx=None):
        ...
        if hasattr(self, "refname"):
            ...
            try:
                result = session.run(
                    Cypher_refname.save_link(link_type),
                    use=self.reftype,
                    a_name=self.name,
                    a_attr=a_attr,
                    b_name=self.refname,
            ...
            #                                       format(a_oid, a_name, a_use, b_oid, b_name))
                ...
                logger.warning(
                    "Could no store (a) -[:{}]-> (b): {}".format(link_type, err)
                )
            ...
            try:
                result = session.run(
                    Cypher_refname.save_single, a_name=self.name, a_attr=a_attr
                )
    ...
    def link_to_refname(tx, pid, name, reftype):
        ...
        #             logger.debug(f"For {name} created {counters}")
            ...
            logging.error(
                f"Refname.link_to_refname: person={pid}, {e.__class__.__name__}, {e}"
            )
            raise
