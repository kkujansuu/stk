...
class Person_combo(Person):
    ...
    def read_my_persons_list(o_context, limit=100):
                ...
                with shareds.driver.session() as session:
                    if show_by_owner:
                        ...
                        if show_with_common:
                            ...
                            result = session.run(
                                Cypher_person.read_all_persons_with_events_starting_name,
                                user=user,
                                start_name=fw_from,
                                limit=limit,
                            )
                            ...
                            result = session.run(
                                Cypher_person.read_my_persons_with_events_starting_name,
                                user=user,
                                start_name=fw_from,
                                limit=limit,
                            )
                        ...
                        result = session.run(
                            Cypher_person.read_approved_persons_with_events_starting_name,
                            start_name=fw_from,
                            limit=limit,
                        )
                        # Returns person, names, events, owners
        ...
        print(
            f"read_my_persons_list: Get max {limit} persons "
        ...
        if persons:
            o_context.update_session_scope(
                "person_scope",
                persons[0].sortname,
                persons[-1].sortname,
                limit,
  ...
  ORDER BY name.order"""
    ...
    def get_person_w_names(self):
        ...
        with shareds.driver.session() as session:
            if self.uuid:
                result = session.run(
                    Cypher_person.get_by_uuid_w_names_notes, pid=self.uuid
                )
            else:
                result = session.run(
                    Cypher_person.get_w_names_notes, my_user=user, pid=self.uniq_id
                )
...
RETURN ID(p1) AS id1, [n1.firstname, n1.suffix, n1.surname] AS name1,
 ...
 RETURN ID(p) AS uniq_id, p, n ORDER BY n.surname, n.firstname"""
    ...
    @staticmethod
    def get_person_w_events(
        keys, args={}
    ):  # , currentuser, take_refnames=False, order=0):
            ...
            try:
                if rule == "uniq_id":
                    ...
                    if order == 1:  # order by first name
                        result = session.run(
                            Cypher_person.get_events_all_firstname, years=years
                        )
                    elif order == 2:  # order by patroname
                        result = session.run(
                            Cypher_person.get_events_all_patronyme, years=years
                        )
                    else:
                    ...
                    if context_code == 1:  # COMMON
                        result = session.run(
                            Cypher_person.get_common_events_by_refname_use,
                            attr={"use": rule, "name": key},
                        )
                    elif context_code == 2:  # OWN
                        result = session.run(
                            Cypher_person.get_my_events_by_refname_use,
                            attr={"use": rule, "name": key},
                            user=user,
                        )
                    else:  # From my OWN Batch & approved COMMON
                        result = session.run(
                            Cypher_person.get_both_events_by_refname_use,
                            attr={"use": rule, "name": key},
                            user=user,
                        )
            except Exception as err:
                print(
                    "iError get_person_w_events: {1} {0}".format(err, keys), file=stderr
                )
    ...
    def get_family_members(uniq_id):
        ...
        return shareds.driver.session().run(
            Cypher_family.get_persons_family_members, pid=int(uniq_id)
        )
    ...
    def get_refnames(pid):
        ...
        # └──────────────────────────┴─────────────────────┘
        print(
            "-> OBSOLETE? Person_combo.get_family_members, use models.gen.refname.Refname.get_refnames?"
        )
        query = """
...
"""
    ...
    def save(self, tx, **kwargs):
        ...
        raise NotImplementedError(
            "Person_combo.save() ei toteutettu, ks. Person_gramps.save()"
        )
