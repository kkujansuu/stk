...
class DbTree:
    ...
    def _get_tree_branches(self, node_id):
        ...
        with self.driver.session(default_access_mode="READ") as session:
            ...
            return [(record["nodes"], record["lv"], record["r"]) for record in result]
            ...
            for node in nodes:
                if not self.tree.contains(node.id):
                    self.tree.create_node(
                        node.get(self.name_field_name),
                        node.id,
                        parent=0,
                        data={
                            self.type_field_name: node.get(self.type_field_name),
                            "uuid": node.get("uuid"),
                        },
                    )
    ...
    def print_tree(self):
        ...
        fill = ""
        nodes = [
            self.tree[node] for node in self.tree.expand_tree(mode=self.tree.DEPTH)
        ]
        for node in nodes:
...
"""
        with driver.session() as session:
            ...
            t.tree.create_node(
                record["name"], locid, parent=0, data={"type": record["type"]}
            )
