        ...
        with shareds.driver.session() as session:
            try:
                ...
                for record in result:
                    ...
                    print(
                        f"do_schema_fixes: Audit links {rel_deleted} removed, {rel_created} added"
                    )
                    if rel_created + rel_deleted > 0:
                        logger.info(
                            f"database.schema_fixes.do_schema_fixes: "
                ...
                if counters.properties_set > 0:
                    logger.info(
                        "database.schema_fixes.do_schema_fixes: profile _Stk_ name set"
                    )
            ...
            #                 print(f"database.schema_fixes.do_schema_fixes: fixed Refname links {cnt1} REFNAME, {cnt2} matronyme, {cnt3} patronyme")
                ...
                logger.error(
                    f"{e} in database.accessDB.do_schema_fixes/Audit"
            ...
            for label in [
                ...
                try:
                    ...
                    logger.warning(
                        f"do_schema_fixes Index for {label}.handle not created."
                        f" Failed {e.__class__.__name__} {e.message}"
                    )
            return

            print(
                f"database.schema_fixes.do_schema_fixes: index updates: {dropped} removed, {created} created"
            )
