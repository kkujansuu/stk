...
class Source_gramps(Source):
    ...
    def save(self, tx, **kwargs):
        ...
        try:
            ...
            result = tx.run(
                Cypher_source_w_handle.create_to_batch, batch_id=batch_id, s_attr=s_attr
            )
            ids = []
            for record in result:
                ...
                if len(ids) > 1:
                    print(
                        "iError updated multiple Sources {} - {}, attr={}".format(
                            self.id, ids, s_attr
                        )
                    )
        ...
        for note_handle in self.note_handles:
            try:
                tx.run(
                    Cypher_source_w_handle.link_note,
                    handle=self.handle,
                    hlink=note_handle,
                )
            except Exception as err:
                logger.error(
                    f"Source_gramps.save: {err} in linking Notes {self.handle} -> {self.note_handles}"
                )
        ...
        for repo in self.repositories:
            try:
                tx.run(
                    Cypher_source_w_handle.link_repository,
                    handle=self.handle,
                    hlink=repo.handle,
                    medium=repo.medium,
                )
