...
neo4j_driver = GraphDatabase.driver(
    neo4j_uri, auth=(neo4j_username, neo4j_password), connection_timeout=15
)
...
def batches():
    ...
    run(
        cypher,
        callback=lambda n, count, rec, kwargs: batchlist.append(dict(rec.get("b"))),
    )
    return batchlist
...
def generate_searchkey1(name_normalizer, n, count, rec, kwargs):
    ...
    for e, pl in rec.get("events"):
        ...
        if etype:
            ...
            if edate:
                ...
                if len(edate) > 4:
                    keys.append(f"E{etype}Y{edate[0:4]}")
            if eplace:
    ...
    # print(searchkey1)
    run(
        "match (p:Person) where id(p) = $pid set p.searchkey1=$searchkey1 return p",
...
def generate_searchkey(n, count, rec, kwargs):
    ...
    index_name = index_name_from_batch_id(batch_id)
    run(
        f"""match (p:Person) where id(p) = $pid 
...
def generate_keys(args):
    ...
    elif args.namematch_algo == "soundex":
        name_normalizer = lambda arg, default: soundex(arg[0])
    elif args.namematch_algo == "hiski":
        name_normalizer = hiskinames.get
    else:
    ...
    run(
        cypher1,
        callback=functools.partial(generate_searchkey1, name_normalizer),
        batch_id=args.for_batch,
    )
    ...
    n = run(
        cypher2,
        callback=generate_searchkey,
        batch_id=args.for_batch,
        namematch_algo=args.namematch_algo,
    )
...
def display_matches(args, p, pid, pn, rec, matches, index_name1, index_name2):
    ...
    namenodes = rec.get(
        "namenodes"
    )  # there may be several Name nodes with order:0, pick the first
    ...
    if 1 or score >= args.minscore:
        ...
        if len(matchkeys.split()) < args.minitems:
            return
        pdict1 = dict(p)
...
def __search_dups(n, count, args, rec, matches):
    ...
    if n % 100 == 0:
        print(f"{time.time()-t0}: Searching {n}/{count}")
    pid = rec.get("pid")
    p = rec.get("p")
    ...
    if len(keys) < args.minitems:
        return

    if args.namematch:
        name = getname(pn).lower()
        if name.find(args.namematch.lower()) < 0:
            return
    ...
    num_matches = run(
        ...
        limit 5
    """,
        callback=lambda n, count, rec, kwargs: display_matches(
            args, p, pid, pn, rec, matches, index_name, index_name2
        ),
        searchkey=searchkey,
        pid=pid,
...
def search_dups(args):
    ...
    """,
        callback=lambda n, count, rec, kwargs: __search_dups(
            n, count, args, rec, matches
        ),
...
def prune_matches(matches):
    def get_firstnames(key, refnames):
        words = key.split()
        names = [
            refnames.get(value[1:], value[1:]) for value in words if value[0] == "G"
        ]
        return set(names)
    ...
    for match in matches:
        ...
        gender1 = p1["sex"]
        gender2 = p2["sex"]
        if gender1 > 0 and gender2 > 0 and gender1 != gender2:
            continue
        ...
        if birth_high1 < birth_low2:
            continue
        if birth_high2 < birth_low1:
            continue
        if death_high1 < death_low2:
            continue
        if death_high2 < death_low1:
            continue
        ...
        if len(common_names) == 0:
            continue
        if common_names != firstnames1 and common_names != firstnames2:
            continue
