...
def fetch_namefamily(basename, usage):
    result = (
        ...
        .run(cypher_fetch_namefamily, lookfor=basename, usage=usage)
        .single()
    )
    if not result:
        return dict(status="Not found", statusText="Not found", resultCount=0, names=[])
    names = result["names"]
...
def add_to_family(basename, names_to_add, usage):
    with shareds.driver.session() as tx:
        for name in names_to_add:
            if name != basename:
                result = tx.run(
                    cypher_add_to_namefamily, basename=basename, name=name, usage=usage
                )
...
def save_name(original_name, name, source):
    result = shareds.driver.session().run(
        cypher_save_name, original_name=original_name, name=name, source=source
    )
    return {
        "status": "OK",
        "statusText": "OK",
