...
def build_general_stats():
    ...
    (route_files, route_lines) = count_files_lines(
        "routes.py", line_pattern=r"^@.*route"
    )
...
def build_options(logname_template, lookup_table):
    ...
    def get_logfiles(log_root, logname_template):
        ...
        files = list(
            filter(os.path.isfile, glob.glob(f"{log_root}/{logname_template}*"))
        )
        # sort by age:
    ...
    def safe_get_request(opt_key, default):
        ...
        try:
            ...
            flash(
                f"Bad number for {opt_key} '{res}': {e}; using default {default}",
                category="warning",
            )
            return default
