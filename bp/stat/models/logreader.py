    ...
    def topn_subcounters(self):
        ...
        if self._opts["bycount"] is not None:
            result = sorted(
                ...
                key=lambda x: (-counter_value(x, "N"), -counter_value(x, "n"), x._name),
            )

        else:
            result = sorted(self._counters.values(), key=lambda x: x._name)
        ...
        if level > 0:
            ...
            for tag, value in self._values.items():
                ...
                if tag == "e" and self._values["N"] != 0:
                    columns.append(
                        f"{number_to_string(1000*value/self._values['N'])} ms"
                    )
...
class StkServerlog(Counter):
    ...
    def parser(self, logfile: str):
        ...
        log_re = re.compile(
            r"(\d\d\d\d-\d\d-\d\d)"  # YYYY-MM-DD (keep this)
...
class StkUploadlog(Counter):
    ...
    def __init__(self, name, level=0, by_what=None, opts=None):
        ...
        self._files = []  # list of files already processed
        self._savers = {}
        (self._want_step_re, self._want_step_if_match) = self.get_regexp_from_opts(
            "msg"
        )
        if by_what is None:
            return
    ...
    def parser(self, logfile: str) -> None:
        ...
        ts1_re = re.compile(
            r"^(\d\d?\.\d\d?.\d\d\d\d)" r" (?:\d\d:\d\d)"  # 1) dmy (txt)
        )  # HH:MM
        ts2_re = re.compile(
