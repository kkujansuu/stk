...
def nodes(label, skip):
    res = runcypher(
        f"match (n:{label}) return n,id(n) as id order by id skip {skip} limit 25"
    )
    # print(res)
...
def search():
    ...
    if pattern:
        ...
        pattern = f"(?i).*{pattern}.*"
        res = runcypher(
            f"match (n:{label})  where n.{attr} =~ $pattern return n,id(n) as id order by id limit 25",
            pattern=pattern,
        )
    else:
...
def update():
    ...
    if type(value) == str and datatype == "number":
        if value == "":
            ...
            value = float(value)
    res = runcypher(
        f"match (n:{label}) where id(n) = $id set n.{attr} = $value", id=id, value=value
    )
...
def fetch(nodeid):
    res = runcypher(
        f"match (n) where id(n) = $nodeid return n,labels(n) as labels", nodeid=nodeid
    ).single()
    print("res1:", res)
...
def fetch_links(nodeid):
    res = runcypher(
        f"match (n)-[r]->(a) where id(n) = $nodeid return *,labels(a) as labels,id(a) as id",
        nodeid=nodeid,
    )
    print(res)
    rsp1 = [
