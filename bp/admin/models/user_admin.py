...
class UserAdmin:
    ...
    def _build_profile_from_record(cls, userProfile):
        ...
        if profile.created_at:
            profile.created_at = datetime.fromtimestamp(
                float(profile.created_at) / 1000
            )
        if profile.approved_at:
            profile.approved_at = datetime.fromtimestamp(
                float(profile.approved_at) / 1000
            )
        return profile
    ...
    def _build_user_from_node(self, userRecord):
        ...
        try:
            ...
            user.roles = [
                rolenode.name
                for rolenode in shareds.user_datastore.find_UserRoles(user.email)
            ]
        ...
        try:
            with shareds.driver.session() as session:
                with session.begin_transaction() as tx:
                    tx.run(
                        Cypher_adm.user_profile_register,
                        name=profile.name,
                        email=profile.email,
    ...
    def update_user_profile(cls, profile):
        try:
            with shareds.driver.session() as session:
                with session.begin_transaction() as tx:
                    tx.run(
                        Cypher_adm.user_profile_update,
                        name=profile.name,
                        email=profile.email,
                        username=profile.username,
                        language=profile.language,
    ...
    def get_accesses():
        try:
            ...
            for rec in shareds.driver.session().run(Cypher_adm.list_accesses):
                ...
                rsp.append(access)
            logger.info(
                f"-> bp.admin.models.user_admin.UserAdmin.get_accesses n={len(rsp)}"
            )
            return rsp
