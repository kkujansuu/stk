...
def load_refnames(pathname):
    ...
    with open(pathname, "r", newline="", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            try:
                ...
                raise KeyError(
                    _(
                        'Not valid field names "Name,Refname,Reftype,Source,Gender" {}'
                    ).format(row.keys())
                )
            ...
            if (refname != "") and (refname != nimi):
                if reftype in REFTYPES:
                    ...
                    logging.warning(
                        "cvs_refnames: Invalid reference {} discarded. ".format(reftype)
                    )
            if sex:
                r.sex = sex
            if source != "":
    ...
    msg = "{}: {} rows, {} skipped. TIME {} sek".format(
        pathname, row_nro, empties, time.time() - t0
    )
    logging.info(msg)
    return msg
