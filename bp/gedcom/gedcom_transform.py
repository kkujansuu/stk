...
def read_gedcom(args):
    ...
    try:
        for linenum, line in enumerate(open(args.input_gedcom, encoding=args.encoding)):
            ...
            if (
                line and line[0] == "\ufeff"
            ):  # UTF-8 BOM (should use utf-8-sig encoding...)
                line = line[1:]
...
def process_gedcom(args, transform_module, task_name=""):
    ...
    LOG.info("------ Ajo '%s'   alkoi %s ------", task_name, util.format_timestamp())
    ...
    if hasattr(transform_module, "process"):
        ...
        try:
            ...
            LOG.error(
                "Ohjelma päättyi virheeseen {}: {}".format(type(err).__name__, str(err))
            )

    LOG.info("------ Ajo '%s' päättyi %s ------", task_name, util.format_timestamp())
...
def main():
    ...
    parser.add_argument(
        "--display-changes", action="store_true", help="Display changed rows"
    )
    parser.add_argument(
        "--dryrun", action="store_true", help="Do not produce an output file"
    )
    parser.add_argument(
        "--nolog", action="store_true", help="Do not produce a log in the output file"
    )
