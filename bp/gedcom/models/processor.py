...
def process_gedcom(arglist, transform_module):
    ...
    parser.add_argument(
        "--display-changes", action="store_true", help=_("Display changed rows")
    )
    parser.add_argument(
        "--dryrun", action="store_true", help=_("Do not produce an output file")
    )
    parser.add_argument(
        "--nolog",
        action="store_true",
    ...
    try:
        ...
        with Output(args) as out:
            ...
            print(
                "<b>------ "
                + ngettext(
                    "Total {num} change", "Total {num} changes", num=t.num_changes
        ...
        if old_name:
            gedcom_utils.history_append(
                args.input_gedcom, _("File saved as {}").format(args.input_gedcom)
            )
            gedcom_utils.history_append(
                args.input_gedcom, _("Old file saved as {}").format(old_name)
            )
        else:
            gedcom_utils.history_append(
                args.input_gedcom,
    ...
    if (
        hasattr(transform_module, "output_format")
        ...
        def build_command_args(self, argdict):
            ...
            for arg in self.args:
                if arg.name in argdict:
                    ...
                    if value:
                        if (
                            arg.action in {"store_true", "store_false"}
                            and value == "on"
                        ):
                            value = ""
                        if arg.name[0] == "-":
