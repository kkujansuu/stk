...
def gedcom_versions(gedcom):
    ...
    versions.append(
        (-1, gedcom, _("Current file"), util.format_date(os.stat(fullname).st_mtime))
    )
...
def gedcom_compare(gedcom1, gedcom2):
    ...
    difftable = HtmlDiff().make_file(
        lines1, lines2, context=True, numlines=2, fromdesc=gedcom1, todesc=gedcom2
    )
...
def gedcom_revert(gedcom, version):
    ...
    if os.path.exists(filename1) and os.path.exists(filename2):
        ...
        gedcom_utils.history_append(
            filename1, _("File {} saved as {}").format(filename1, newname)
        )
        gedcom_utils.history_append(
            filename1, _("File {} saved as {}").format(filename2, filename1)
        )
...
def gedcom_upload():
    ...
    if file and allowed_file(file.filename):
        ...
        logger.info(
            f"-> bp.gedcom.routes.gedcom_upload n={os.stat(fullname).st_size/1024.}kb"
        )
...
def gedcom_download(gedcom):
    ...
    if gedcom_utils.get_gedcom_user() != current_user.username and not metadata.get(
        "admin_permission"
    ):
        flash(
            _("You don't have permission to view that GEDCOM"), category="flash_error"
        )
    ...
    return send_from_directory(
        directory=gedcom_folder, filename=gedcom, as_attachment=True
    )
...
def gedcom_info(gedcom):
    ...
    if not os.path.exists(filename):
        flash(
            _("That GEDCOM file does not exist on the server"), category="flash_error"
        )
        return redirect(url_for("gedcom.gedcom_list"))
...
def gedcom_update_permission(gedcom, permission):
    ...
    gedcom_utils.save_metadata(gedcom, metadata)
    logger.info(
        f'-> bp.gedcom.routes.gedcom_update_permission/{permission} f="{gedcom}"'
    )
    return "ok"
...
def get_excerpt(gedcom, linenum):
    ...
    for i, line in enumerate(lines[firstline : linenum + 9]):
        line = line.strip()
        if i > 0 and firstline + i > linenum - 1 and line.startswith("0"):
            break
...
def gedcom_delete(gedcom):
    ...
    for name in os.listdir(gedcom_folder):
        if name.endswith("-history"):
            continue  # do not remove history
        if (
            name == gedcom
            or name.startswith(gedcom + ".")
            or name.startswith(gedcom + "-")
        ):
...
def gedcom_transform(gedcom, transform):
    ...
    if request.method == "GET":
        ...
        return render_template(
            "gedcom_transform_params.html",
            gedcom=gedcom,
            transform=transform,
            transform_name=transform_module.name,
            rows=rows,
        )
    else:
        ...
        tr_args = "{} {} {} {} {}".format(
            transform[:-3], gedcom_filename, args, "--logfile", logfile
        )
        cmd3 = "cd '{}'; PYTHONPATH='{}' {} {} {}".format(
            gedcom_app, python_path, python_exe, transform_py, tr_args
        )
        ...
        p.wait()
        if s2:
            gedcom_utils.history_append(gedcom_filename, "\nErrors:\n" + s2)
        #         s = "\n" + _("Errors:") + "\n" + s2 + "\n\n" + s1
        try:
