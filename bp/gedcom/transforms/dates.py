...
doclinks = {"fi": "http://wiki.isotammi.net/wiki/Gedcom_työkalut_Päivämäärät"}


def add_args(parser):
    parser.add_argument(
        "--display_invalid_dates", action="store_true", help=_("Display invalid dates")
    )
    parser.add_argument(
        "--handle_dd_mm_yyyy",
        action="store_true",
    ...
    parser.add_argument(
        "--handle_yyyy_mm", action="store_true", help=_("Change 1999-12 to DEC 1999")
    )
    parser.add_argument(
        "--handle_zeros", action="store_true", help=_("Change [00.]12.1999 to DEC 1999")
    )
    parser.add_argument(
        "--handle_zeros2", action="store_true", help=_("Change 00 DEC 1999 to DEC 1999")
    )
    parser.add_argument(
        "--handle_intervals",
        action="store_true",
    ...
    def transform(self, item, options, phase):
        ...
        if item.tag == "DATE":
            ...
            if options.handle_zeros:
                ...
                # 00.00.1888 -> 1888
                r = match(
                    value, z1=zerototwozeros, dot=dot, z2=zerototwozeros, y=fourdigits
                )
                if r:
                    item.value = r.y
                    return item
            ...
            if options.handle_yyyy_mm_dd:
                # 1888-12-31
                r = match(
                    value, y=fourdigits, sep=dash, m=twodigits, sep2=dash, d=twodigits
                )
                if r:
                    val = fmtdate(r.y, r.m, r.d)
                    if val:
