        ...
        if len(lpart) > 0:
            ...
            if re.match(regexb, lpart):
                ...
                cstring = "{} {}".format(
                    ...
                    src_groups[7].strip() if src_groups[7] else "",
                )
...
class Citations(transformer.Transformation):
    ...
    def transform(self, item, options, phase):
        ...
        if phase == 1:
            ...
            if item.level > 0 and path.endswith(
                ".SOUR"
            ):  # SOUR referenced by an element
                    ...
                    if len(ntitletexts) == 1:
                        ...
                        for ind in range(0, len(referrers)):
                            if ncitations[0] == " ":
                                continue
                            citation = (
                                "{} PAGE ".format(str(referrers[ind][1] + 1))
                                + ncitations[0]
                            )
                            self.insertions[referrers[ind][0]] = [
                                transformer.Item(citation)
                            ]
                    ...
                    #                            format(referrers[ind][0], self.insertions[referrers[ind][0]]))
                        ...
                        for ind in range(0, len(ntitletexts)):
                            ...
                            if ind < len(referrers):
                                sourceref = "{} SOUR {}".format(
                                    referrers[ind][1], sourceid
                                )
                                citation = (
                                    "{} PAGE ".format(str(referrers[ind][1] + 1))
                                    + ncitations[0]
                                )
                                self.replaces[referrers[ind][0]] = [
                                    transformer.Item(sourceref),
                                    transformer.Item(citation),
                                ]
