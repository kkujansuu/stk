    ...
    def transform(self, item, options, _phase):
        ...
        if options.remove_invalid_marriage_dates:
            if item.line.strip() == "1 MARR":
                ...
                #     1 MARR
                #     2 TYPE AVOLIITTO
                if len(item.children) > 0 and item.children[0].line.startswith(
                    "2 DATE AVOLIITTO"
                ):
        ...
        if options.remove_invalid_divorce_dates:
            if item.line.strip() == "1 DIV":
                ...
                # with
                #     1 DIV Y
                if len(item.children) == 1 and item.children[0].line.startswith(
                    "2 DATE ."
                ):
                    return Item(
                        "1 DIV Y"
                    )  # this is not valid GEDCOM but Gramps will fix it
        ...
        if options.remove_empty_notes:  # 2.1.1
            if (
                ...
                and allempty(item.children)
            ):
                return None

        if options.remove_empty_dates:  # 2.2.4
            if item.tag == "DATE" and item.value.strip() in ("", ".", "?"):
                return None
        ...
        if options.fix_addr:  # 5.1.2
            if item.tag == "ADDR" and item.value.strip() != "":
                ...
                item.children.insert(
                    0, Item("%s ADR1 %s" % (item.level + 1, item.value))
                )
                item.value = ""
                return item
        ...
        if options.fix_events:  # 5.1.4
            if item.tag == "EVEN" and len(item.children) == 2:
                ...
                if (
                    c1.tag == "TYPE"
                    and c1.value in ("Ei_julkaista", "Kummit", "Tutkijan omat")
                    and c2.tag == "PLAC"
                ):
                    c2.tag = "NOTE"
                    if c1.value == "Kummit":
                        c2.value = "Description: " + c2.value
                    return item
...
def add_args(parser):
    ...
    parser.add_argument(
        "--remove_empty_dates", action="store_true", help=_("Remove invalid DATE tags")
    )
    parser.add_argument(
        "--remove_empty_notes", action="store_true", help=_("Remove empty NOTE tags")
    )
    parser.add_argument(
        "--remove_invalid_marriage_dates",
        action="store_true",
        help=_("Remove DATE AVOLIITTO tags"),
    )
    parser.add_argument(
        "--remove_invalid_divorce_dates",
        action="store_true",
        help=_("Remove invalid DATEs for DIV tags"),
    )
    parser.add_argument(
        "--remove_empty_nameparts",
        action="store_true",
    ...
    parser.add_argument(
        "--remove_refn", action="store_true", help=_("Remove REFN tags")
    )
    parser.add_argument(
        "--remove_stat", action="store_true", help=_("Remove STAT tags")
    )
    parser.add_argument(
        "--save_level_3_notes",
        action="store_true",
    ...
    parser.add_argument(
        "--fix_addr", action="store_true", help=_("Insert ADR1 tags under ADDR")
    )
    parser.add_argument(
        "--fix_events",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=_(
            "Remove trailing and multiple consecutive spaces in person and place names"
        ),
    )
    parser.add_argument(
        "--emig_to_resi", action="store_true", help=_("Change EMIG to RESI")
    )
    parser.add_argument(
        "--note_to_page",
        action="store_true",
