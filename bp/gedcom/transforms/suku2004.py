...
def add_args(parser):
    parser.add_argument(
        "--change_custom_tags_to_even",
        action="store_true",
        help=_("Change custom tags to EVENs"),
    )
    parser.add_argument(
        "--change_photo_to_obje",
        action="store_true",
    ...
    parser.add_argument(
        "--combine_plac_and_addr", action="store_true", help=_("Combine PLAC and ADDR")
    )
        ...
        if options.change_custom_tags_to_even:
            if item.tag in custom_tags:
                ...
                if len(item.children) == 0:
                    return None  # empty _UNKN tag, remove
                return item
        ...
        if options.change_marr_to_marb:
            if item.tag == "MARR":
                if len(item.children) > 0 and (
                    item.children[0].tag == "TYPE"
                    and item.children[0].value.lower() in {"kuulutus", "kuulutettu"}
                ):
                    item.tag = "MARB"
                    return item
