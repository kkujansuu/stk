...
class SurnameParser:
    ...
    def _parse_surnames_no_parens(self, surn):
        ...
        i = surn.find("(")
        if i >= 0:
            raise ParseError(f"Expected no '(': '{surn}'")
        i = surn.find(")")
        if i >= 0:
            raise ParseError(f"Expected no ')': '{surn}'")
        for s in TYPES.keys():
            surn = surn.replace(
                f" {s}", f" {s} "
            )  # separate "os.Mäkinen" -> "os. Mäkinen"
            surn = surn.replace(
                f",{s}", f" {s} "
            )  # separate "Virtanen,os. Mäkinen" -> "Virtanen, os. Mäkinen"
            if s[-1] == ".":
                surn = surn.replace(
                    f",{s[:-1]} ", f" {s} "
                )  # separate "Virtanen,os Mäkinen" -> "Virtanen, os Mäkinen"
            ...
            for surname in surnames2:
                ...
                for name in names:
                    ...
                    if len(parts) > 1 and parts[0] in PREFIXES:
                        ...
                        prefix = ""
                    if name:
                        surnames.append(SurnameInfo(name, name_type, prefix))
