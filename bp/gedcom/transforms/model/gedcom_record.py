...
class GedcomRecord(GedcomLine):
    ...
    def __str__(self):
        return "GedcomRecord {} {} ({} riviä)".format(
            self.path, self.value, len(self.rows)
        )
...
if __name__ == "__main__":
    ...
    logging.basicConfig(
        filename="example.log", level=logging.DEBUG, format="%(levelname)s:%(message)s"
    )
