...
def add_args(parser):
    ...
    parser.add_argument(
        "--patronyme_in_given_name", action="store_true", help=_("Recognize_patronymes")
    )
    parser.add_argument(
        "--call_names", action="store_true", help=_("Call names and aliases")
    )
    parser.add_argument(
        "--create_surname_history",
        action="store_true",
        help=_("Create surname history from multiple names"),
    )
    parser.add_argument(
        "--aliases", action="store_true", help=_("Process_nonstd_ALIA_lines")
    )
...
class PersonNames(transformer.Transformation):
    def transform(self, item, _options, _phase):
            ...
            for pn in sorted(parseresult.surnames, key=self.surname_sortkey):
                ...
                if first:
                    if n.patronymic_conflict:
                        item3 = Item(
                            f"{subitem.level+1} NOTE _W {_('patronyymiristiriita')}"
                        )
                    ...
                    if n.nsfx:
                        ...
                        if (
                            orig_nsfx
                            and orig_nsfx != n.nsfx
                            and not n.patronymic_conflict
                        ):
                            item3 = Item(
                                f"{subitem.level+1} NOTE _W {_('patronyymiristiriita 2')}"
                            )
                            newitem.children.append(item3)
                if pn.prefix:
        ...
        if (
            item.tag in ["NPFX", "GIVN", "NICK", "SPFX", "SURN", "NSFX"]
            and item.value == ""
            and item.children
        ):
    ...
    def surname_sortkey(self, surnameinfo):
        ...
        if surnameinfo.name_type == surnameparser.Name_types.BIRTH_NAME:
            return 0
        if surnameinfo.name_type is None:
            return 1
        return 2
