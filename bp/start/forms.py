...
class JoinForm(FlaskForm):
    ...
    name = StringField(
        _l("Your name:"), [InputRequired()], description=_l("Name of the user")
    )
    email = StringField(
        _l("Your email address:"),
        [
            InputRequired(),
            Email(),
    ...
    GSF_membership = RadioField(
        _l("Are you a member of the Genealogical Society of Finland?"),
        [Optional()],
        choices=[
            ("yes", _l("Yes")),
            ("no", _l("No")),
    ...
    research_years = StringField(
        _l("How many years have you been doing genealogical research?")
    )
    software = StringField(_l("What genealogy software are you mainly using?"))
    researched_names = StringField(
        _l("Which families/surnames are you mainly researching?")
    )
