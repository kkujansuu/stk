...
class Test(unittest.TestCase):
    ...
    def testNodeObject_sort(self):
        ...
        for i in range(len(nodesorted)):
            self.assertEqual(
                nodesorted[i].uniq_id, expected_uids[i], f"Invalid sort order, pos {i}"
            )
    ...
    def testDate_compare(self):
        ...
        # If self < other?
        self.assertEqual(
            mydate.__lt__(DateRange(DR["DATE"], "1650")), True, "Not 1645 < 1650"
        )
        self.assertEqual(
            mydate.__eq__(DateRange(DR["DATE"], "1645")), True, "Not 1645 == 1645"
        )
        self.assertEqual(
            mydate.__gt__(DateRange(DR["DATE"], "1640")), True, "Not 1645 > 1640"
        )
