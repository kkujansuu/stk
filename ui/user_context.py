...
class UserContext:
    ...
    class ChoicesOfView:
        ...
        def __init__(self):
            ...
            self.as_str = {
                ...
                self.COMMON
                + self.BATCH: N_("My selected batch and approved common data"),
            }
    ...
    def __init__(self, user_session, current_user=None, request=None):
        ...
        if request:
            ...
            new_selection = int(request.args.get("div", 0)) + int(
                request.args.get("div2", 0)
            )
            if new_selection:
    ...
    def set_scope_from_request(
        ...
        if self.session_var:
            ...
            print(
                f"UserContext.set_scope_from_request: {self.session_var} is cleared {scope}"
            )
        return
    ...
    def update_session_scope(self, var_name, name_first, name_last, limit, rec_cnt):
        ...
        """
        print(
            f"UserContext.update_session_scope: Got {rec_cnt} items {name_first!r} – {name_last!r}, "
        ...
        if scope_old[0] != self.first or scope_old[1] != self.last:
            print(
                f"update_session_scope: New {var_name!r} {self.first!r} – {self.last!r}"
            )
    ...
    def next_name(self, direction="fw"):
        ...
        print(
            f'UserContext.next_name: {[self.first, self.last]}, {direction} next="{ret}"'
        )
        return ret
