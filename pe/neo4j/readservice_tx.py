...
class Neo4jReadServiceTx(ConcreteService):
    ...
    def tx_get_person_list(self, args):
        ...
        try:
            if restart:
                ...
                if show_approved:
                    print(
                        f"tx_get_person_list: Show approved, common data fw={fw_from}"
                    )
                    result = self.tx.run(
                ...
                if show_approved:
                    print(
                        f'tx_get_person_list: Show approved common data {rule} ~ "{key}*"'
                    )
                    result = self.tx.run(
                ...
                if show_approved:
                    print(
                        f"tx_get_person_list: Show approved common data years {years}"
                    )
                    result = self.tx.run(
            ...
            for record in result:
                ...
                p.events_w_role = record.get(
                    "events"
                )  # list of tuples (event_node, place_name, role)
    ...
    def tx_get_person_by_uuid(self, uuid: str, active_user: str):
        ...
        try:
            ...
            if active_user is None:
                if root_type != "PASSED":
                    print(
                        f"dx_get_person_by_uuid: person {uuid} is not in approved material"
                    )
                    res.update(
                        {
                            "status": Status.NOT_FOUND,
    ...
    def tx_get_person_families(self, puid: int):
        ...
        try:
            ...
            for record in results:
                ...
                family_role = record[
                    "role"
                ]  # Member role as: father / mother / None (child)
                ...
                family_members = []
                for parental_role, member_node, name_node, event_node in record[
                    "members"
                ]:
    ...
    def tx_get_object_places(self, base_objs: dict):
        ...
        try:
            ...
            return {
                "status": Status.ERROR,
                "statustext": f"{e.__class__.__name__}: {e}",
            }
    ...
    def tx_get_object_citation_note_media(self, obj_catalog: dict, active_objs=[]):
        ...
        try:
            ...
            results = self.tx.run(
                CypherPerson.get_objs_citations_notes_medias, uid_list=uids
            )
            for record in results:
                # Returns
                #    - src      source node
            ...
            msg = (
                f"Could not read 'Citations, Notes, Medias': {e.__class__.__name__} {e}"
            )
            print(f"dx_get_object_citation_note_media: {msg}")
            return {"status": Status.ERROR, "statustext": msg}

        return {"status": Status.OK, "new_objects": new_obj_ids, "references": coll}
    ...
    def tx_get_object_sources_repositories(self, citation_uids: list):
        ...
        * On return, the new Sources and Repositories found are added to objs{}
        ...
        with self.driver.session(default_access_mode="READ") as session:
            results = session.run(
                CypherSource.get_citation_sources_repositories, uid_list=citation_uids
            )
            for record in results:
                # <Record
                #    uniq_id=392761
