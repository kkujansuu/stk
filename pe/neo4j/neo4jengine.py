...
class Neo4jEngine:
    ...
    def __init__(self, app):
        ...
        print(
            f'Neo4jEngine: {app.config["NEO4J_USERNAME"]} connecting (v>={self.version})'
        )
    ...
    def consume_counters(self, result):
        ...
        try:
            ...
            logger.error(
                "pe.neo4j.neo4jengine.Neo4jEngine.consume_counters:"
                f"Invalid Neo4j database version, expected {self.version}"
            )
            raise NotImplementedError(
                "Wrong version, expecting Neo4j v" + self.version
            ) from e
